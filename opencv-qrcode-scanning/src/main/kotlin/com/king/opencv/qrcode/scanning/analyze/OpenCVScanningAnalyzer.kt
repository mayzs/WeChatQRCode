...
import com.king.opencv.qrcode.DecodeResult
...
open class OpenCVScanningAnalyzer @JvmOverloads constructor(
    ...
    private fun detectAndDecode(nv21: ByteArray, frameMetadata: FrameMetadata): AnalyzeResult<DecodeResult>? {
        ...
        try {
            val decodeResult = detector.detectAndDecodeResult(bgrMat, isOutputVertices, isDetectMultiple)
            return if (decodeResult.texts.isNotEmpty()) {
                AnalyzeResult(nv21, ImageFormat.NV21, frameMetadata, decodeResult)
            } else {
                // 空结果时主动释放点位 Mat，避免无意义持有 native 内存
                decodeResult.points?.release()
                null
            }
        } finally {
            bgrMat.release()
